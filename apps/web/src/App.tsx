...
/* ===================== Estilos ===================== */
const pageWrap: React.CSSProperties = { fontFamily:"Inter, system-ui, Segoe UI, Roboto, Arial", padding:16, lineHeight:1.5, color:"#0f172a", background:"#f8fafc" };
...
/* ===================== Balance por tipo (MVP) ===================== */
const EFFECT_DEFAULTS: Record<string, { value?: number; duration?: number; cooldown: number }> = {
  DANO:     { value: 80,  cooldown: 2 },
  HEAL:     { value: 60,  cooldown: 2 },
  ESCUDO:   { value: 70,  cooldown: 3 },
  DOT:      { value: 20,  duration: 3, cooldown: 4 },
  HOT:      { value: 20,  duration: 3, cooldown: 4 },
  STUN:     {              duration: 1, cooldown: 3 },
  SILENCE:  {              duration: 1, cooldown: 3 },
  MARCACAO: {              duration: 2, cooldown: 2 }, // placeholder
};

/* ===================== Catálogo ===================== */
function mkSkill(id:string, name:string, target:TargetTeam, effects:ActiveSkill["effects"], cost:ActiveSkill["cost"]): ActiveSkill {
  const fixed = effects.map(e => {
    const d = (EFFECT_DEFAULTS as any)[e.kind];
    return {
      ...e,
      ...(d && d.value    !== undefined ? { value: d.value } : {}),
      ...(d && d.duration !== undefined ? { duration: d.duration } : {}),
    };
  });
  const cd = Math.max(1, ...fixed.map(e => (((EFFECT_DEFAULTS as any)[e.kind]?.cooldown) ?? 1)));
  return { id, name, target, effects: fixed, cost, cooldown: cd };
const KIT_A: ActiveSkill[] = [
  mkSkill("a_1", "Raio", "ENEMY", [{kind:"DANO"}], { AZUL:2 }),
  mkSkill("a_2", "Barreira", "SELF", [{kind:"ESCUDO"}], { AZUL:1, BRANCO:1 }),
  mkSkill("a_3", "Atordoar", "ENEMY", [{kind:"STUN"}], { BRANCO:2 }),
  mkSkill("a_4", "Incendiar", "ENEMY", [{kind:"DOT"}], { VERMELHO:2 }),
];
const KIT_B: ActiveSkill[] = [
  mkSkill("b_1", "Golpe", "ENEMY", [{kind:"DANO"}], { VERMELHO:2 }),
  mkSkill("b_2", "Guarda", "SELF", [{kind:"ESCUDO"}], { AZUL:2 }),
  mkSkill("b_3", "Silenciar", "ENEMY", [{kind:"SILENCE"}], { BRANCO:2 }),
  mkSkill("b_4", "Bênção", "ALLY", [{kind:"HOT"}], { VERDE:2 }),
];
const KIT_C: ActiveSkill[] = [
  mkSkill("c_1", "Flecha", "ENEMY", [{kind:"DANO"}], { VERMELHO:2 }),
  mkSkill("c_2", "Cura", "ALLY", [{kind:"HOT"}], { VERDE:2 }),
...
/* ===================== UI Aux: Chips de Energia ===================== */
const ENERGY_META: Record<string,{label:string; icon:string; bg:string; border:string}> = {
  AZUL:      { label:"Azul",      icon:"🔵", bg:"#eff6ff", border:"#93c5fd" },
  VERMELHO:  { label:"Vermelho",  icon:"🔴", bg:"#fef2f2", border:"#fca5a5" },
  VERDE:     { label:"Verde",     icon:"🟢", bg:"#ecfdf5", border:"#6ee7b7" },
  BRANCO:    { label:"Branco",    icon:"⚪", bg:"#f8fafc", border:"#cbd5e1" },
  PRETA:     { label:"Preta",     icon:"⚫", bg:"#f1f5f9", border:"#94a3b8" },
};
...
function EnergyChips({pool}:{pool:Record<string,number>}) {
  ...
  const chip: React.CSSProperties = { display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 10px", borderRadius:12, fontWeight:700, border:"1px solid #e5e7eb" };
  ...
  return (
    <div style={wrap}>
      {Object.entries(ENERGY_META).map(([k,m]) => (
        <div key={k} style={{...chip, background:m.bg, borderColor:m.border}}>
